...
const listener = app.listen(settings.website.port, () => {
  ...
  console.log(chalk.green(`Version: ${chalk.white(settings.version)}`));
  console.log(chalk.blue(`Theme: ${chalk.white(settings.defaulttheme)}`));
  console.log(chalk.cyan(`Dashboard: ${chalk.underline(settings.api.client.oauth2.link + "/")}`));
});
...
const REPO_OWNER = "lapsusdevs";
const REPO_NAME = "Cupual";
...
async function checkForUpdates() {
    try {
        ...
        const response = await fetch(GITHUB_API, {
            headers: { "User-Agent": "Cupual-Updater" },
        });
...
async function downloadUpdate(url, newVersion) {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const options = { headers: { "User-Agent": "Cupual-Updater" } };
...
async function getFinalRedirectUrl(url) {
  return new Promise((resolve, reject) => {
      https.get(url, { headers: { "User-Agent": "Cupual-Updater" } }, (response) => {
...
function restartApplication() {
  ...
  exec("pm2 restart Cupual", (error, stdout, stderr) => {
      if (error) {
